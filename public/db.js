  ...
  const Auth = {
    init() {
      if (!localStorage.getItem(KEYS.ADMINS)) {
        saveAll(KEYS.ADMINS, DEFAULT_ADMINS);
      }
    },
    ...
    login(username, password) {
      ...
      return admins.find(a => a.username === username && a.passwordHash === hash) || null;
    },

    addAdmin(username, password, nom, role = 'staff') {
      const all = this.getAdmins();
      if (all.find(a => a.username === username)) return false;
      all.push({ id: generateId('ADM'), username, passwordHash: simpleHash(password), nom, role });
      saveAll(KEYS.ADMINS, all);
      return true;
    },

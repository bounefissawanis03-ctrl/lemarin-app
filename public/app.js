...
function initHeroImages() {
  ...
  document.getElementById('layerAerial').classList.add('active');
}
...
// ─── DRONE SHOW SECTION (PARALLAX SCROLL) ─────────────────
function initDroneShow() {
  ...
  window.addEventListener('scroll', () => {
    ...
    imgs.forEach(img => {
      img.style.transform = `scale(${1 + progress * 0.05}) translateY(${progress * 30}px)`;
    });
  });
}
...
function initGallery() {
  ...
  // First 10 are user's real photos, rest are stock
  GALLERY_IMAGES.forEach((img, idx) => {
    const item = document.createElement('div');
    // Alternate large/tall/normal layout
    let cls = '';
    if (idx === 0 || idx === 10) cls = 'large';
    else if (idx === 3 || idx === 8 || idx === 15) cls = 'tall';
  ...
  slider?.addEventListener('input', () => {
    ...
    const saturate = 0.3 + (val / 100) * 0.7;
    const items = document.querySelectorAll('.gallery-item');
    items.forEach(item => {
      item.style.filter = `brightness(${brightness}) saturate(${saturate})`;
      const overlay = item.querySelector('.gallery-night-overlay');
      if (overlay) {
        overlay.style.opacity = (1 - val / 100) * 0.6;
      }
    });
    // Update label
    const dn = document.querySelector('.dn-label');
  });
}
...
function initScrollReveal() {
  const els = document.querySelectorAll(
    '.room-card, .wt-scene, .info-item, .feature-item, .loc-item, .walkthrough-header, .rooms-header, .gallery-header, .time-container, .section-heading'
  );
...
function initDayNightToggle() {
  ...
  btn?.addEventListener('click', () => {
    ...
    const imgs = document.querySelectorAll('.hero-img');
    if (isDay) {
      document.getElementById('heroImgAerial').src = PHOTOS.hotel_day;
      document.getElementById('heroImgEntrance').src = PHOTOS.lounge_day;
    } else {
      document.getElementById('heroImgAerial').src = PHOTOS.exterior_night;
      document.getElementById('heroImgEntrance').src = PHOTOS.entrance_night;
    }
  });
}
...
function submitBooking(e) {
  ...
  const data = {
    ...
    heure:       new Date().toLocaleTimeString('fr-DZ'),
  ...
  if (msgEl) msgEl.textContent = `Bonjour ${data.prenom} ${data.nom}! Votre réservation #${data.id} pour le ${data.checkin} a été enregistrée.`;
...
function renderReviews() {
  ...
  if (!carousel) return;
  const reviews = LM_DB.Reviews.getApproved();
...
function updateGlobalRating() {
  const avg = LM_DB.Reviews.getAvgRating();
...
function submitReview(e) {
  ...
  LM_DB.Reviews.add({ prenom, ville, note, commentaire: text });
  renderReviews();
  updateGlobalRating();
  e.target.reset();
